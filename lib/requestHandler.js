  ...
  return function (req, userRes) {
    ...
    */
    ...
    co(fetchReqData)
      ...
      .then(
        co.wrap(function* () {
          const userModifiedInfo =
            (yield userRule.beforeSendRequest(Object.assign({}, requestDetail))) || {}
      ...
      // route user config
      .then(
        co.wrap(function* (userConfig) { // userConfig就是finalReqDetail
...
function getConnectReqHandler(reqHandlerCtx, userRule, recorder, httpsServerMgr) {
  return function (req, socket) {
    ...
    if (host.includes("trackh5.guahao.cn")) {
      debugger
    }
    let resourceInfo
    let resourceInfoId
    ...
    function _sendFailedSocket(error) {
      ...
      socket.write(
        "HTTP/" + req.httpVersion + " 502 Proxy Inner Error\r\n" + errorHeader + "\r\n\r\n"
      )
    }
    ...
    co(function* () {
          ...
          const serverSocket = net.connect(serverInfo.port, serverInfo.host, () => {
            socket.write("HTTP/" + req.httpVersion + " 200 OK\r\n\r\n", "UTF-8", () => {
              ...
              if (global._throttle && !shouldIntercept) {
                serverSocket.pipe(global._throttle.throttle()).pipe(socket)
      ...
      .catch(
        co.wrap(function* (error) {
          logUtil.printLog(
            "error happend when dealing https req:" + error + "  " + host,
            logUtil.T_ERR
          )
