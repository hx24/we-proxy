...
export default {
  ...
  computed: {
    ...
    filteredDataList() {
      const { dataList, inputKeyWord, selectKeyWord } = this
  ...
  methods: {
    filterKey() {
      // monitor.devtool('埋点查看-搜索', {
      //   input: this.inputKeyWord,
      //   select: this.selectKeyWord
      // })
      if (!this.inputKeyWord) {
        this.curDataList = this.dataList
      } else if (this.selectKeyWord === 'click_event') {
        this.curDataList = this.dataList.filter((item) => {
          return item.event && item.event.includes(this.inputKeyWord)
        })
      } else if (this.selectKeyWord === 'click_id') {
        this.curDataList = this.dataList.filter((item) => {
          return item.clickId && String(item.clickId).includes(this.inputKeyWord)
        })
      } else {
        this.curDataList = this.dataList.filter((item) => {
          const hasEvent = item.event && item.event.includes(this.inputKeyWord)
