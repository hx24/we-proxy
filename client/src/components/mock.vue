<template>
  <div id="mock">
    <div class="mock-project">
      <el-dialog :visible.sync="addProjectStatus">
        <div slot="title">添加项目</div>
        <el-input type="input" v-model="newProjectName"></el-input>
        <div slot="footer" class="dialog-footer">
          <el-button @click="addProjectStatus = false">取消</el-button>
          <el-button type="primary" @click="addProject">保存</el-button>
        </div>
      </el-dialog>
      <el-button type="primary" :plain="true" icon="el-icon-plus" @click="addProjectStatus = true">添加项目</el-button>
      <ul class="mock-project-list el-menu el-menu-vertical-demo">
        <li class="mock-project-item el-menu-item" :class="{ 'is-active': item.id === currentProject.id }" :key="item.id" v-for="item in projects" @click="switchProject(item)">
          {{ item.name }}
    ...
    <div class="mock-path" v-if="currentProject.id">
      <el-dialog size="large" :visible.sync="addPathStatus">
        <div slot="title">添加接口</div>
        ...
        <div slot="footer" class="dialog-footer">
          <el-button @click="cancelPath">取消</el-button>
      ...
      <el-table class="mock-path__list" :data="projectPaths" border @selection-change="handlePathChange">
        <el-table-column type="selection" width="55"> </el-table-column>
        <el-table-column prop="request.url" label="Url" width="400"> </el-table-column>
        <el-table-column prop="request.method" label="Method" width="100"> </el-table-column>
...
export default {
  ...
  methods: {
    ...
    switchProject(item) {
      ...
      this.$remoteApi.getProjectPaths(item.id).then(
        (data) => {
