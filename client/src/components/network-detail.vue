  ...
  <div id="network_detail">
    <el-collapse v-model="activeNames" v-show="panelType === 'headers'" key="1">
      ...
      <el-collapse-item title="Response Header" name="resheaders">
        <div v-for="(value, key) in headers.resHeader" :key="key">
          <b>{{ key }}: </b>{{ value }}
        </div>
      </el-collapse-item>
      <el-collapse-item title="Request Headers" name="reqheaders">
        <template v-for="(value, key) in headers.reqHeader">
          <div v-if="key !== 'Cookie'" :key="key">
            <b>{{ key }}: </b>{{ value }}
          </div>
        </template>
      </el-collapse-item>
    ...
    preview() {
      ...
      if (this.panelType === 'preview') {
        ...
        if (type.match('json')) {
          ...
          return highlightCache[detail.id] || (highlightCache[detail.id] = '<pre><code>' + hljs.highlightAuto(detail.content).value + '</code></pre>')
