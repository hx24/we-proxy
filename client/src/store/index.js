...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    [types.UPDATE_RECORDER](state, newrecorder) {
      //渲染列表不存储所有值，提高渲染速度
      let filterRecorder = newrecorder.slice(state.recorder_len, newrecorder.length).map((item) => {
        return _.pick(item, ["id", "method", "statusCode", 'url', "host", "path", "mine", "startTime"])
      })
  ...
  getters: {
    ...
    monitorRecords: (state) => {
        const records = state.recorder.reduce((res, record) => {
